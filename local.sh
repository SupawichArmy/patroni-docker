...
ssh adminn01@10.1.3.19 "cd ~/patroni-docker; git fetch ; git pull; docker build -t patroni1 -f Dockerfile-node1 .; docker compose -f docker-compose-node1.yml down; docker compose -f docker-compose-node1.yml up -d --build; docker compose -f docker-compose-haproxy.yml up -d --build " &
